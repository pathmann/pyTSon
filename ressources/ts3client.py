...
class Config(object):
    """
    Offers an interface to query the TeamSpeak 3 client's config database (settings.db).
    You should always del a reference to this object if not needed anymore to assure the database connection is closed.
    """
...
class IconPack(object):
    ...
    _prefixes = ["200x200_", "100x100_", "32x32_", "24x24_", "20x20_", "16x16_", "8x7_", "7x5_", ""]

    _varNames = ['3D_SOUND', '3D_SOUND', '3D_SOUND_ME', 'ABOUT', 'ACTIVATE_MICROPHONE', 'ADD', 'ADD_FOE', 'ADD_FRIEND', 'ADDON',
                 'ADDON_COLLECTION', 'ADDON_BROWSER', 'ADDON_ICONPACK', 'ADDON_SOUNDPACK', 'ADDON_THEME', 'ADDON_TRANSLATION', 'ARROW_LEFT',
                 'ARROW_RIGHT', 'AWAY', 'AWAY', 'AWAY', 'BAN_CLIENT', 'BAN_CLIENT', 'BAN_LIST', 'BOOKMARK_ADD', 'BOOKMARK_ADD_FOLDER',
                 'BOOKMARK_DUPLICATE', 'BOOKMARK_MANAGER', 'BOOKMARK_MANAGER', 'BOOKMARK_REMOVE', 'BROKEN_IMAGE', 'BROKEN_IMAGE', 'CAPTURE',
                 'CAPTURE', 'CAPTURE', 'CHANGE_NICKNAME', 'CHANGELOG', 'CHANNEL_CHAT', 'CHANNEL_COLLAPSE_ALL', 'CHANNEL_COMMANDER',
                 'CHANNEL_CREATE', 'CHANNEL_CREATE', 'CHANNEL_CREATE', 'CHANNEL_CREATE_SUB', 'CHANNEL_CREATE_SUB', 'CHANNEL_DELETE',
                 'CHANNEL_DELETE', 'CHANNEL_EDIT', 'CHANNEL_EXPAND_ALL', 'CHANNEL_GREEN', 'CHANNEL_GREEN_SUBSCRIBED', 'CHANNEL_PRIVATE',
                 'CHANNEL_RED', 'CHANNEL_RED_SUBSCRIBED', 'CHANNEL_SWITCH', 'CHANNEL_YELLOW', 'CHANNEL_YELLOW_SUBSCRIBED', 'CHECK_UPDATE',
                 'CONNECT', 'CONNECT', 'COPY', 'DEFAULT', 'DELETE', 'DELETE_AVATAR', 'DISCONNECT', 'DISCONNECT', 'DOWN', 'DOWN_ARROW', 'EDIT',
                 'EDIT_FRIEND_FOE_STATUS', 'EDIT_FRIEND_FOE_STATUS', 'EMOTICON', 'ERROR', 'FILE_HOME', 'FILE_REFRESH', 'FILE_UP',
                 'FILETRANSFER', 'FILETRANSFER', 'FIND', 'GROUP_100', 'GROUP_200', 'GROUP_300', 'GROUP_500', 'GROUP_600', 'GUISETUP',
                 'GUISETUP', 'HARDWARE_INPUT_MUTED', 'HARDWARE_INPUT_MUTED', 'HARDWARE_INPUT_MUTED', 'HARDWARE_OUTPUT_MUTED',
                 'HARDWARE_OUTPUT_MUTED', 'HARDWARE_OUTPUT_MUTED', 'HOSTER_BUTTON', 'HOTKEYS', 'HOTKEYS', 'ICONSVIEW', 'IDENTITY_ADD',
                 'IDENTITY_DEFAULT', 'IDENTITY_EXPORT', 'IDENTITY_IMPORT', 'IDENTITY_MANAGER', 'IDENTITY_MANAGER', 'IDENTITY_REMOVE', 'INFO',
                 'INPUT_MUTED', 'INPUT_MUTED', 'INPUT_MUTED', 'INPUT_MUTED_LOCAL', 'INPUT_MUTED_LOCAL', 'INPUT_MUTED_LOCAL', 'IS_TALKER',
                 'KICK_FROM_CHANNEL', 'KICK_FROM_SERVER', 'LISTVIEW', 'LISTVIEW', 'LOADING_IMAGE', 'MESSAGE_INCOMING', 'MESSAGE_INCOMING',
                 'MESSAGE_INFO', 'MESSAGE_INFO', 'MESSAGE_OUTGOING', 'MESSAGE_OUTGOING', 'MODERATED', 'MUSIC', 'MYTS_ACCOUNT_ONLINE',
                 'MYTS_ACCOUNT_OFFLINE', 'MYTS_ACCOUNT_ERROR', 'MYTS_ACCOUNT', 'MYTS_ACCOUNT', 'NEW_CHAT', 'NOTIFICATIONS', 'NOTIFICATIONS',
                 'OFFLINE_MESSAGES', 'ON_WHISPERLIST', 'OUTPUT_MUTED', 'OUTPUT_MUTED', 'OUTPUT_MUTED', 'PERMISSION_OVERVIEW',
                 'PERMISSIONS_CHANNEL_CLIENT', 'PERMISSIONS_CHANNEL_GROUPS', 'PERMISSIONS_CHANNELS', 'PERMISSIONS_CLIENTS',
                 'PERMISSIONS_SERVER_GROUPS', 'PHONETICSNICKNAME', 'PING_1', 'PING_2', 'PING_3', 'PING_4', 'PING_CALCULATING', 'PING_DISCONNECTED',
                 'PLAY', 'PLAYBACK', 'PLAYBACK', 'PLAYBACK', 'PLAYER_CHAT', 'PLAYER_CHAT', 'PLAYER_COMMANDER_OFF', 'PLAYER_COMMANDER_ON',
                 'PLAYER_OFF', 'PLAYER_OFF', 'PLAYER_OFF', 'PLAYER_ON', 'PLAYER_ON', 'PLAYER_ON', 'PLAYER_WHISPER', 'PLAYER_WHISPER',
                 'PLAYER_WHISPER', 'PLUGINS', 'POKE', 'PRESENT', 'QUIT', 'RECORDING_START', 'RECORDING_STOP', 'REGISTER', 'REMOVE_FOE',
                 'REMOVE_FRIEND', 'RELOAD', 'SECURITY', 'SELECTFOLDER', 'SEND_COMPLAINT', 'SERVER_GREEN', 'SERVER_LOG', 'SERVER_QUERY', 'SETTINGS',
                 'SETUP_WIZARD', 'STOP', 'SUBSCRIBE_TO_ALL_CHANNELS', 'SUBSCRIBE_TO_ALL_CHANNELS', 'SUBSCRIBE_TO_CHANNEL', 'SYNC_ENABLE',
                 'SYNC_DISABLE', 'SYNC', 'TAB_CLOSE_BUTTON', 'TEMP_SERVER_PASSWORD', 'TEMP_SERVER_PASSWORD_ADD', 'TEXTFORMAT', 'TEXTFORMAT_BOLD',
                 'TEXTFORMAT_FOREGROUND', 'TEXTFORMAT_ITALIC', 'TEXTFORMAT_UNDERLINE', 'TOGGLE_BLOCK_INCOMING_WHISPERS',
                 'TOGGLE_SERVER_QUERY_CLIENTS', 'TOKEN', 'UNSUBSCRIBE_FROM_ALL_CHANNELS', 'UNSUBSCRIBE_FROM_ALL_CHANNELS', 'UNSUBSCRIBE_FROM_CHANNEL',
                 'UP', 'UP_ARROW', 'UPLOAD_AVATAR', 'URLCATCHER', 'URLCATCHER', 'VIRTUALSERVER_EDIT', 'VOLUME', 'WARNING', 'WEBLIST', 'WHISPERLISTS',
                 'WHISPERLISTS', 'WHISPERLISTS', 'WARNING_EXTERNAL_LINK']
    ...
    def __init__(self, *, info=None, name=None):
        ...
        if name:
            cfg = Config()
            q = cfg.query("SELECT value FROM addons WHERE value LIKE '%%name=%s%%'" % name)

            if not q.next():
                del cfg
                raise Exception("No iconpack with name %s found in database" % name)
    ...
    def open(self):
        ...
        if self.zip:
            ...
            if not "settings.ini" in os.listdir(self.path) or not os.path.isfile(os.path.join(self.path, "settings.ini")):
                raise Exception("No settings.ini in iconpack found")
...
TALK_POWER_REVOKE_ALL_GRANT_NEXT
    ...
    def fallback(self, var):
        ...
        if self.settings.has_option("options", "FALLBACK") and self.settings.getboolean("options", "FALLBACK"):
            try:
                with IconPack(info="default") as defpack:
                    return defpack.icon(var)
            except:
                return QPixmap()
        elif self.settings.has_option("options", "PLACEHOLDER") and self.settings.getint("options", "PLACEHOLDER") == 2:
            ret = QPixmap(32, 32)
